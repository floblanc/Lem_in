...
/*   Updated: 2019/05/01 17:45:45 by floblanc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		way_is_possible(int **matrix, t_room *tab, t_path *new, int way)
{
	...
	while (lim > 0)
	{
		...
		{
			lim--;
			if (tab[i].taken == 0 && tab[i].used != way + 1
	...
	while (i < best->len[best->path_n -1])
	{
		if (best->node[best->path_n - 1][i] > 0)
		{
			if ((count -= matrix[best->node[best->path_n - 1][i]]
						[best->node[best->path_n - 1][i]]) < 0
					&& way_is_possible(matrix, tab, *new, best->path_n - 1))
			{
				node++;
				return (i);
			}
