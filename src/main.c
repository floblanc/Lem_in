...
/*   Updated: 2019/05/10 15:25:47 by floblanc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	main3(int **matrix, t_room *tab, int size)
{
	...
	int		i;
	int		j;
//must create "last_best" or "global_best", rewrite or modify "copy_best"
	j = 2;
	...
	i = -1;
	while (++i < size)
		printf("tab[%d](%s).taken = %d wth = %d degre = %d\n", i, tab[i].name, tab[i].taken, tab[i].wth, matrix[i][i]);
